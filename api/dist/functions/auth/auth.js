...
var import_nodemailer = require("../../lib/nodemailer/nodemailer");
const handler = async (event, context) => {
  ...
  const signupOptions = {
    ...
    handler: async ({
      ...
      const verificationToken = (0, import_node_crypto.randomUUID)();
      await import_db.db.user.create({
        data: {
          email: username,
          hashedPassword,
          salt,
  ...
  const authHandler = new import_auth_dbauth_api.DbAuthHandler(event, context, {
    ...
    cookie: {
      ...
      Path: "/",
      SameSite: "Strict",
      Secure: !import_env.isDevelopment,
      // If you need to allow other domains (besides the api side) access to
      // the dbAuth session cookie:
      Domain: import_env.isDevelopment ? "localhost" : `${process.env.DOMAIN_NAME}`
    },
